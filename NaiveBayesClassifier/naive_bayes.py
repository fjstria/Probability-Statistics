...
# set of all the distinct tokens in the file.
# =============================================================
...
class NaiveBayes():
    ...
    def fit(self, train_hams: list, train_spams: list):
        ...
        def get_counts(filenames: list):
            ...
            self.word_counts_ham!
            """
            num_emails = 0  # number of emails that fit label
            word_counts = {}  # dictionary of words : count of times appeared
            for file in filenames:  # for each file in the list filenames
                num_emails += 1  # count this file
                for word in self.word_set(file):  # for each word in file
                    if word not in word_counts:
                        word_counts[word] = 1  # if new word, add to dict and count it
                    else:
                        word_counts[word] += 1  # if word exists, increment count by 1
                    # END IF-ELSE
                # END INNER FOR
            # END OUTER FOR
            return num_emails, word_counts  # return as tuple

        # END get_counts()

        self.num_train_hams, self.word_counts_ham = get_counts(train_hams)  # obtain and set num_emails, word_counts
        self.num_train_spams, self.word_counts_spam = get_counts(train_spams)
    ...
    def predict(self, filename: str):
        ...
        prob_spam = np.log(self.num_train_spams / (self.num_train_hams + self.num_train_spams))  # log(P(spam))
        prob_ham = np.log(self.num_train_hams / (self.num_train_hams + self.num_train_spams))  # log(P(ham))

        prob_word_spam = 0  # vars to hold summation
        prob_word_ham = 0

        for word in self.word_set(filename):
            # for each word in the file, sum log(P(word|spam/ham)) with laplace smoothing
            prob_word_spam += np.log((self.word_counts_spam.get(word, 0) + 1) / (self.num_train_spams + 2))
            # summation for spam
            prob_word_ham += np.log((self.word_counts_ham.get(word, 0) + 1) / (self.num_train_hams + 2))
            # summation for ham
        # END for

        # conditional is the comparison from assignment description, true if overall P(email is spam) > P(email is ham)
        # log(P(spam)) + sum[i=1 to k](log(P(word[i]|spam))) > log(P(ham)) + sum[i=1 to k](log(P(word[i]|ham)))
        if (prob_spam + prob_word_spam) > (prob_ham + prob_word_ham):
            return self.SPAM_LABEL  # condition is met, predict spam
        else:
            return self.HAM_LABEL  # if not spam, it's ham (ties get this label)
        # END if-else
